...
#define ERROR_SUCCESS @"E_SUCCESS"
...
WX_EXPORT_METHOD(@selector(openPicker:callback:))
WX_EXPORT_METHOD(@selector(openCropper:callback:))
WX_EXPORT_METHOD(@selector(clean:))
WX_EXPORT_METHOD(@selector(cleanSingle:callback:))
...
- (void) setConfiguration:(NSDictionary *)options callback:(WXModuleCallback)callback {
...
- (void)openCamera:(NSDictionary *)options callback:(WXModuleCallback)callback {
    [self setConfiguration:options callback:callback];
    self.currentSelectionMode = CAMERA;
#if TARGET_IPHONE_SIMULATOR
    self.callback(@{@"code":ERROR_PICKER_CANNOT_RUN_CAMERA_ON_SIMULATOR_KEY, @"message":ERROR_PICKER_CANNOT_RUN_CAMERA_ON_SIMULATOR_MSG});
    return;
#else
    [self checkCameraPermissions:^(BOOL granted) {
        if (!granted) {
            self.callback(@{@"code":ERROR_PICKER_NO_CAMERA_PERMISSION_KEY, @"message":ERROR_PICKER_NO_CAMERA_PERMISSION_MSG});
            return;
        }
...
- (void)imagePickerControllerDidCancel:(UIImagePickerController *)picker {
    [picker dismissViewControllerAnimated:YES completion:[self waitAnimationEnd:^{
        self.callback(@{@"code":ERROR_PICKER_CANCEL_KEY,@"message":ERROR_PICKER_CANCEL_MSG});
    }]];
}
...
- (void)cleanSingle:(NSString *) path callback:(WXModuleCallback)callback {
    BOOL deleted = [[NSFileManager defaultManager] removeItemAtPath:path error:NULL];
    if (!deleted) {
        callback(@{@"code":ERROR_CLEANUP_ERROR_KEY,@"message":ERROR_CLEANUP_ERROR_MSG});
    } else {
        callback(@{@"code":ERROR_SUCCESS});
    }
}

- (void)clean:(WXModuleCallback)callback {
...
- (void)openPicker:(NSDictionary *)options callback:(WXModuleCallback)callback {
    [self setConfiguration:options callback:callback];
    self.currentSelectionMode = PICKER;
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
        if (status != PHAuthorizationStatusAuthorized) {
            self.callback(@{@"code":ERROR_PICKER_UNAUTHORIZED_KEY,@"message":ERROR_PICKER_UNAUTHORIZED_MSG});
            return;
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            // init picker
...
- (void) openCropper:(NSDictionary *)options callback:(WXModuleCallback)callback {
    [self setConfiguration:options callback:callback];
    self.currentSelectionMode = CROPPING;
    NSString* path = [options objectForKey:@"path"];
    UIImage* img =  [[UIImage alloc]initWithContentsOfFile:path];
    [self startCropping:img];
    //self.callback(@{ERROR_CROPPER_IMAGE_NOT_FOUND_KEY:ERROR_CROPPER_IMAGE_NOT_FOUND_MSG});
}
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            ...
                            if (video == nil) {
                                ...
                                [imagePickerController dismissViewControllerAnimated:YES completion:[self waitAnimationEnd:^{
                                    self.callback(@{@"code":ERROR_CANNOT_PROCESS_VIDEO_KEY,@"message":ERROR_CANNOT_PROCESS_VIDEO_MSG});
                                }]];
                                return;
                            }
                         ...
                         dispatch_async(dispatch_get_main_queue(), ^{
                             ...
                             @autoreleasepool {
                                 ...
                                 if (filePath == nil) {
                                     ...
                                     [overlayView removeFromSuperview];
                                     [imagePickerController dismissViewControllerAnimated:YES completion:[self waitAnimationEnd:^{
                                         self.callback(@{@"code":ERROR_CANNOT_SAVE_IMAGE_KEY,@"message":ERROR_CANNOT_SAVE_IMAGE_MSG});
                                     }]];
                                     return;
                                 }
            ...
            if (phAsset.mediaType == PHAssetMediaTypeVideo) {
                [self getVideoAsset:phAsset completion:^(NSDictionary* video) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        [imagePickerController dismissViewControllerAnimated:YES completion:[self waitAnimationEnd:^{
                            if (video != nil) {
                                self.callback(@{@"code":ERROR_SUCCESS,@"data":video});
                            } else {
                                self.callback(@{@"code":ERROR_CANNOT_PROCESS_VIDEO_KEY,@"message":ERROR_CANNOT_PROCESS_VIDEO_MSG});
                            }
                        }]];
...
- (void) processSingleImagePick:(UIImage*)image withExif:(NSDictionary*) exif withViewController:(UIViewController*)viewController withSourceURL:(NSString*)sourceURL withLocalIdentifier:(NSString*)localIdentifier withFilename:(NSString*)filename {
    ...
    if (image == nil) {
        [viewController dismissViewControllerAnimated:YES completion:[self waitAnimationEnd:^{
            self.callback(@{@"code":ERROR_PICKER_NO_DATA_KEY,@"message":ERROR_PICKER_NO_DATA_MSG});
        }]];
        return;
    }
    ...
    if ([[[self options] objectForKey:@"cropping"] boolValue]) {
        ...
        [viewController dismissViewControllerAnimated:YES completion:[self waitAnimationEnd:^{
            self.callback(@{@"code":ERROR_SUCCESS,@"data":[self createAttachmentResponse:filePath
                                                                                withExif:exif
